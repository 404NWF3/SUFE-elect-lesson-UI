...
class CourseSelector:
    ...
    def operate_lessons(self, elect_lesson_id: str, withdraw_lesson_id: str) -> Tuple[bool, str]:
        """执行选课/换课操作

        Returns:
            Tuple[bool, str]: (是否成功, 响应消息)
                - True: 选课成功，任务可以移除
                - False: 选课失败（人数已满等），任务应该保留继续重试
        """
        if not self.cookies or not self.profile_id:
            raise Exception("请先设置cookies和profile_id")
        ...
        try:
            ...
            for failure_msg in failure_messages:
                if failure_msg in response_text:
                    # response成功但选课失败，返回False表示需要保留任务
            ...
            while self.selector.is_running and self.task_list:
                ...
                for i, task in enumerate(self.task_list):
                    ...
                    if sc < lc:
                        ...
                        # 执行选课操作
                        success, response = self.selector.operate_lessons(elect_id, withdraw_id)

                        # 保存响应到调试文件
                        with open("operateLessons_debug.txt", "a", encoding="utf-8") as f:
                            f.write(f"\n\n=== {datetime.now()} ===\n")
                            f.write(f"选课ID: {elect_id}, 退课ID: {withdraw_id}\n")
                            f.write(f"选课结果: {'成功' if success else '失败'}\n")
                        ...
                        if success:
                            ...
                            # 选课失败（人数已满、点击过快等），保留任务继续重试
                            task['status'] = '选课失败，继续重试'
                            self.root.after(0, self.refresh_task_list)
                            self.log(f"✗ 选课失败，将继续重试: {self.selector.get_course_full_info(elect_id)}", "WARN")
                    else:
                        task['status'] = '已满'
                        self.root.after(0, self.refresh_task_list)
